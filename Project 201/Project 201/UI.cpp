...
void UI::interface(Station** array, int num) const {
	...
	if (array[num]->Buses_Mixed_Forward.Count() > 0) {
		for (int i = 0; i < array[num]->Buses_Mixed_Forward.Count(); i++) {
			std::cout << "B" << array_mix_f[i]->getId() << "[FWD, MBus, " << array_mix_f[i]->get_capacity() << "]" << std::endl;
		}
	}
	if (array[num]->Buses_Mixed_Backward.Count() > 0) {
		for (int i = 0; i < array[num]->Buses_Mixed_Backward.Count(); i++) {
			std::cout << "B" << array_mix_b[i]->getId() << "[BCK, MBus, " << array_mix_b[i]->get_capacity() << "]" << std::endl;
		}
	}
	if (array[num]->Buses_Wheel_Forward.Count() > 0) {
		for (int i = 0; i < array[num]->Buses_Wheel_Forward.Count(); i++) {
			std::cout << "B" << array_wheel_f[i]->getId() << "[FWD, WBus, " << array_wheel_f[i]->get_capacity() << "]" << std::endl;
		}
	}
	if (array[num]->Buses_Wheel_Backward.Count() > 0) {
		for (int i = 0; i < array[num]->Buses_Wheel_Backward.Count(); i++) {
			std::cout << "B" << array_wheel_b[i]->getId() << "[BCK, WBus, " << array_wheel_b[i]->get_capacity() << "]" << std::endl;
		}
	}
	std::cout << "----------------------------------------------------------------------" << std::endl;
	std::cout << number_buses_zero_station << " In-Checkup buses: " << std::endl;
	std::cout << "----------------------------------------------------------------------" << std::endl;
